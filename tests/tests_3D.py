...
def perform_ls(path=THREE_D_PATH):
...
def pretty_print():
    with open(os.path.join(ROOT_PATH, 'logs_3D.txt'), 'w') as f:
        for key, value in result_dict.items():
            f.write(key + ': ' + str(value) + '\n')

def main():
    # Load JSON data from a file
    with open(os.path.join(ROOT_PATH, 'testsuite_3D.json'), 'r') as file:
        data = json.load(file)

    print("\nstart 3D testing...\n")

    # Simplified the test iteration into a single loop
    for path, mesh_type in [(THREE_D_PATH, '3D')]:
        ls = perform_ls(path)
        dict_tests = {}
        for item in data:
            for filepath in ls:
