...
def check_ok(mesh, test, mesh_type):
    ...
    # Print all the information from the test object
    print("\n")
    print("File:   ", test.filename)
    print("Swap:   ", test.swap)
    print("Insert: ", test.insert)
    print("Move:   ", test.move)
    print("Hausd:  ", test.hausd)
    print("Hgrad:  ", test.hgrad)
    print("Hmin:   ", test.hmin)
    print("Hmax:   ", test.hmax)
    print("Choice: ", test.choice)
    print("Sandbox:", [{"left": elt['left'], "right": elt['right']} for elt in test.sandbox])
    print("Default:", test.default)
...
def pretty_print():
    with open(os.path.join(ROOT_PATH, 'logs_3DS.txt'), 'w') as f:
        for key, value in result_dict.items():
            f.write(key + ': ' + str(value) + '\n')

def main():
    # Load JSON data from a file
    with open(os.path.join(ROOT_PATH, 'testsuite_3DS.json'), 'r') as file:
        data = json.load(file)

    print("\nstart 3DS testing...\n")

    # Simplified the test iteration into a single loop
    for path, mesh_type in [(SURFACE_PATH, 'surface')]:
